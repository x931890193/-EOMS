...
def vrvcheck():
	...
	for server in services.keys():
		for s in services.get(server):
			s["port"] = s.get("port")
			if server == 'cpp' or (server == 'base' and s.get('name') in (
				'redis-server', 'turnserver', 'fdfs_trackerd', 'fdfs_storaged', 'nginx')):
				...
				'zookeeper', 'mysql', 'kafka', "mongodb", "apache2", 'elasticsearch', 'tomcat-webapp',
				'tomcat-app',):
